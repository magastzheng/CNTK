...
namespace CNTK
{
    ...
    void CompositeFunction::GetNetworkOutputs(std::unordered_map<Variable, ValuePtr>& outputs)
    {
        ...
        for (auto outputVarValuePair : outputs)
        {
            ...
            if (noValueStrorageProvided && packedVarValue && packedVarValue->IsPacked())
                m_existingNetworkOutputReferences[node] = packedVarValue;
        }
    }
    ...
    void CompositeFunction::GetNetworkGradients(std::unordered_map<Variable, ValuePtr>& gradients)
    {
        ...
        for (auto gradientVarValuePair : gradients)
        {
            ...
            auto packedVarValue = std::dynamic_pointer_cast<PackedValue>(valuePtr);
            if (noValueStrorageProvided && packedVarValue && packedVarValue->IsPacked())
                m_existingNetworkGradientReferences[computationNodePtr] = packedVarValue;
        }
    }
                                                            ...
                                                            const std::unordered_set<Variable>& inputsToExcludeGradientsFor)
    {
        ...
        ClearExistingOutputOrGradientStorageReferences(outputsToEvaluate, /*clearGradientReferences =*/ false);
                                                 ...
                                                 std::unordered_map<Variable, ValuePtr>& backPropagatedGradientValuesForInputs)
    {
        ...
        // Free any previous references to the matrix storage associated with the gradients of nodes that
        // have been requested gradients for
        std::vector<ComputationNodeBasePtr> nodesGradientsRequestedFor;
        for (auto gradVarValuePair : backPropagatedGradientValuesForInputs)
            nodesGradientsRequestedFor.push_back(m_variableToNodeMap.at(gradVarValuePair.first));

        ClearExistingOutputOrGradientStorageReferences(nodesGradientsRequestedFor, /*clearGradientReferences =*/ true);
